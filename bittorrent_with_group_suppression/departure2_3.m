        ...
        if pM(loc,2)==n_pieces
            ...
            for k=1:pM(loc,3)
                ...
                if ismember(pM(loc,1),pM(disconn_row,7:10))
                    yer=find(pM(disconn_row,7:10)==pM(loc,1));
                    pM(disconn_row,6+yer)=0;
                    pM(disconn_row,10+yer)=0;
                end
                
            end
            %we deleted its connections and halted its uploading process to other
            %peers
            %now update the active peers
