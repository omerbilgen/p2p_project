...
for i=1:active_n_p
    ...
    for j=1:4
        if pM(i,6+j)>0
            ...
            peer_id=pM(i,6+j);
            piece_id=pM(i,10+j);
            %row_id is the row the downloading peer is located
...
for i=2:active_n_p
    for j=1:pM(i,3)
        %we now update ith peer's connections'upload rates and other
        %related quantities if the connections have not uploaded to this
        %ith peer in this round
...
%Recall that unlike the implementation of this function in Bittorrent_like, 
%we will use fourth 80-column wide block of the seed's row to store number 
%of pieces that every connection of the seed has.
