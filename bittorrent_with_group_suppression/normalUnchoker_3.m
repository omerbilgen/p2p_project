...
largest=ifLargest(pM, myRow,active_n_p,n_p);
if mod(pM(myRow,end),3)==0
    ...
    while done==0 && pass_count < pM(myRow,3)
        ...
        if largest==1 && pM(row_id,2)<=pM(myRow,2)
            go_ahead=0;
        end
        if ~ismember(pM(myRow,14+n_p+pass_count),pM(myRow,7:10)) && ...
                pM(row_id,end-1)<40  && go_ahead==1
...
%note that if unchoking round is not a multiple of 3, we keep the same 
...
while counter2 < 4-numel(unchoked) && pass_count < n_c
    ...
    if go_ahead3==1
        ...
        interesting=(ones(1,n_p)-not_needed).*pM(myRow,15:14+n_p);
        bbb=cumsum(interesting);
        if bbb(n_p)>0 && pM(row_id,end-1)<40 && ~ismember(pM(row_id,1),unchoked)
...
%choose the pieces to be uploaded
for j=1:numel(unchoked)
    ...
    %the unchoked peer updates its rarity then finds the rarest one among 
    %the pieces that we offer it
