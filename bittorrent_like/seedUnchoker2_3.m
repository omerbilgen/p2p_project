...
for i=1:4
    if pM(1,6+i)>0
        ...
        %subtract 5 from number of downloads at a time
...
%if there is one, we have to sort them based on their upload ratio to the
%network
...
%we need 4-number of unchoked peers so far

%first randomize the order of the connection so that if there is equality
%on their upload rate, which we expect happens a lot since we assumed
%constant uploading rate, the peers with tie are unchoked in random fashion

permute_everyone=randperm(pM(1,3));
temp=pM(1,14+n_p+1:14+n_p+pM(1,3));
pM(1,14+n_p+1:14+n_p+pM(1,3))=temp(permute_everyone);

temp=pM(1,14+n_p+n_max_c+1:14+n_p+n_max_c+pM(1,3));
pM(1,14+n_p+n_max_c+1:14+n_p+n_max_c+pM(1,3))=temp(permute_everyone);

temp=pM(1,14+n_p+n_max_c*2+1:14+n_p+n_max_c*2+pM(1,3));
pM(1,14+n_p+n_max_c*2+1:14+n_p+n_max_c*2+pM(1,3))=temp(permute_everyone);

%sort based on uplad ratios and apply the same ordering to the connections 
...
%if number of unchoked peers from previous round is below 4, we are going
...
%if unchoking counter is not a multiple of 3 and we have 4 candidate peers
%based on the last time they have been unchoked and upload ratio to the
%network, we keep first three but change the fourth one if there exists an
%interested peer outside these four peers

%if unchoking counter is multiple of three, we simply keep all of them.
if mod(pM(1,end),3)~=0
   if numel(unchoked_peers_sorted) ==4
       a1=randperm(pM(1,3));
       done=0;
...
%update unchoked connections and tlast according to unchoked_peers_sorted
%to do that use newly_unchoked_counter especially tlast

%set all uploading connections to 0
pM(1,7:14)=0;
for i=1:numel(unchoked_peers_sorted)
    ...
    %not update if this peer was already unchoked in the previous round
